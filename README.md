...
- **长期记忆**：对它说「记住…」的内容跨会话保存（本机 `logs/memory.json`），下次新开对话自动想起；设置里「清空记忆」一键清除
...
- **长期记忆**：说「记住我喜欢简短回答」这类话，它会存进本机长期记忆（`logs/memory.json`），之后每次新开对话都直接生效，不用重复交代；在设置里点「清空记忆」可一键清除全部（语音说「清空对话」只清当前这轮，不动长期记忆）
