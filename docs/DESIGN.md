...
> **定位（v4 伙伴版）**：给通用 Agent（DeepSeek Harness，DSH）装上语音前端，并在此基础上长成「生活在电脑里的助手、朋友、伙伴」。本文档负责**技术架构**（状态机/路由/桥/审批/风险）；产品愿景与设计哲学见 `PRODUCT.md`，里程碑全量设计（M0-M6：出网安全网关/记忆双轨/八态姿态/主动引擎/观察会话/家居接入/成长记录）见 `ROADMAP.md`，对话级验收见 `specs/EVAL.md`。
>
...
>
> 一句话定位：给通用 Agent 装上语音前端，做成「语音控制的 Agent 工作台」——并以此为底座向伙伴版演进。
>
> 修订史：v2 新增「风险与应对」「语音审批」「长任务异步」三节；v2.1 吸收外部分析（DSH 版本更正 `0.1.1-rc.2`、审批栈源码核实 headless fail-closed、§9 补并发任务与失败/超时反馈）；v2.2 审批更新为**双层结构**——预测式语音审批 + 运行时审批桥 `xiao-approval-bridge`（已落地），A2 收窄为纯流式桥；v2.3 A2 web 流式桥落地（§7 重写为 A1/A2 双桥、§8 补 web 模式审批语义与 XIAO_GRANT fail-closed）；**v4 对齐伙伴版路线图**——定位更新、新增 §11 架构演进层。

## 0. 架构演进层（伙伴版新增，随里程碑落地）
...
- **配套接口**：`/api/audio/devices`（sounddevice 枚举）、`/api/tts/preview`（试听）、`/api/memory/list`（跨会话长期记忆只读清单）、`/api/memory/clear`（一键清空 Agent 历史 + DSH 上下文 + 跨会话长期记忆）、`/api/provider/test`（服务商连通性测试：按环节发最小请求，无效 Key/超额/超时各回一句人话，不抛堆栈）、`/api/health/probe`（健康状态灯：并行探测 ASR/LLM/TTS 当前激活方案 + 检查本机 dsh 命令 + 离线就绪（v3：纯配置判四环节是否全本地），返回各环节绿/红 + 延迟 + 一句人话原因）。
...
- **端侧离线链路自检（v3 已落地）**：`backend/offline.py` 纯配置判定四环节是否全为本地引擎（唤醒 sherpa/omni、识别 local FunASR/omni、大脑 ollama/omni、播报 piper/omni；方案内字段优先，旧单字段配置回退），零网络秒回；作为灯组一盏（`offline_item`）随 `/api/health/probe` 返回，红灯点名「哪个环节还不是本地方案」并引导去设置切换。本地服务（Ollama/vLLM-omni）是否已启动属运行时连通性，仍由灯组其他项探测——配置检查与连通探测分工不重复。
- **长期记忆（v3 已落地）**：`backend/memory.py` **只做显式记忆**——用户明确说「记住…」时由 `remember` 工具（schema 限定仅在明确要求时调用，防模型乱存）写入 `logs/memory.json`（原子写 tmp + replace，文件损坏重置为空不卡启动；FIFO 上限 `memory.max_entries` 默认 100 条），不做 LLM 自动沉淀（噪音大、成本高）。Agent `_messages()` 构建消息时把最近 `memory.inject_limit`（默认 20）条并入系统提示词，新会话自动「想起」；`context_text()` 每次实时读配置，改条数即热生效。设置面板「清空记忆」复用 `/api/memory/clear` → `pipeline.clear_memory()` 扩展为同时清长期记忆（语音说「清空对话/退下」只清当前对话，语义分工：按钮=全部记忆，语音=本轮对话）。
