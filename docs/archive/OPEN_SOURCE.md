...
> 状态：**已开源到 GitHub（glaincer-lab/Xiao，MIT）**。密钥处置见 §二（泄露 key 已作废并轮换；git 历史重写暂缓）；素材版权已确认无风险；本文件随开发持续维护。
...
- **事故**：`config.yaml` 曾明文写入阿里云百炼 Key，并随公开仓库历史提交（`3a3e9d7` 起），任何 clone 者可读。泄露 key 的片段不在本文档复述。
- **处置**：① `config.yaml` 明文 key 全部清空，改走 `.env`（已 gitignore）；② 该泄露 key 已在阿里云后台**作废**；③ 新 key 已写入本机 `.env` 并验证可用。
- **遗留**：开源前已做全历史密钥扫描与清理（4 重验证，git 历史无 key，见 §九）；旧 key 已作废，无资金风险。DeepSeek key 仅在本机 `.env`，未进仓。
...
- [x] 轮换阿里云 key（泄露 key 已作废，新 key 已写入 .env；经过见 §二）
- [x] LICENSE（MIT）
- [x] NOTICE（模型/依赖许可声明）
