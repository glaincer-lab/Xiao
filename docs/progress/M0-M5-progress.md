...
- [x] **全量测试**：`python -m unittest discover -s tests` → 实测 **Ran 815 tests**（failures=2, errors=9）。
- [x] **已知失败 11 项**：均为沙箱既有环境项（`test_memory` 临时目录写权限 + `test_dsh_web_bridge` 时序），**非实现问题**，见 §七。
- [x] **无回落判据**：M3-M0 开工前基线 = **616 tests**（11 项失败）；M3-M5 完成后 = **815 tests**（新增 199，失败仍为同样 11 项）。
...
| **M3 主动引擎** | 已实现（7 包） | **138** | 注意力传感器 / 预算制消费 / 心跳 / 事件触发 / DORMANT 联动+纪念日 / 风格画像 / 影子期 |
| **M4 观察会话** | 已实现（4 包） | **28** | 观察会话框架 S0-S6 / 穿搭场景 / 视线对齐 / 反馈闭环 |
| **M5 物理编排** | 已实现（6 包） | **33** | HA 主路径 / 审批分级 / 场景编排 / VLM 兜底 / 行程编排 / 米家直连(实验默认关) |

**核心合计（M0-M5）：143 + 116 + 115 + 138 + 28 + 33 = 573**（M0 含注意力传感器 +22 时）。

### 1.2 全量测试口径说明（重要，防误读）

- **全量 `discover -s tests` = 815**：包含**所有** `tests/` 下的测试文件（含 T7 编排 / T8 审计 / gateway 分文件 / backup 等非里程碑核心模块，共 264 项）。
- **M0-M2 核心**=352（M0 121 / M1 116 / M2 115）；**M3-M5 核心**=199（M3 138 / M4 28 / M5 33）。
- 三者关系：352（M0-M2 核心）+ 199（M3-M5）+ 264（T7/T8/gateway/backup 等其他）= **815**。
- 历史基线：M3-M0 开工前全量 = **616**（= 352 + 264，注意力传感器尚未实现，M0 实为 121）；M3-M5 完成后 = **815**（新增 199）。
...
| M5-M5 行程/饮食编排 | `backend/m5/trip.py` | A型范式(≤3方案各带代价+置信度→敢推荐→落地动作明确/缺则明说不落地)；外部天气失败降级明说缺什么 | `tests/test_m5_trip.py` 5 |
...
## 六、已知沙箱失败项（非实现问题，审计须知）

全量 `discover -s tests` 的 11 项失败/错误均为**沙箱既有环境项**，与本阶段 M3/M4/M5 实现无关，判据如下（实测 `Ran 815, failures=2, errors=9`）：

| 测试文件 | 失败数 | 根因 | 是否本阶段引入 |
|---|---|---|---|
| `test_memory` | 8 errors + 1 fail | 写入系统临时目录 `C:\\Users\\...\\Temp\\dsh-*` 被受限沙箱拒绝（PermissionError） | 否（开工前基线即存在） |
| `test_dsh_web_bridge.StreamTests.test_e2e_stream_and_steps` | 1 fail | 事件流时序脆弱（tool/call vs work_step 顺序） | 否（既有） |

**无回落判据**：若 M3-M0 开工前基线为 616（11 项失败），M3-M5 后为 815（仍为同样 11 项失败），且失败项名称/数量不变，即判定**无回落**。
