...
| 3 | 存储方案 | 三层记忆：短期全量 2 年（730 天）/ 长期向量 10 年 / 超长期遗忘；总预算 300MB（v4.4.0 由 250MB 修正）；占用超限弹窗确认 |
...
## 六、待实现清单（前 3 项已于 2026-08-31 落地，见 docs/progress/M1-vector-memory-progress.md）

> ✅ 已完成（v2.0 进度基线）：向量库阶段一 / 存储治理落地 / routes 脱敏 / Sleeptime 调度 / 存储满弹窗 / 巩固增量 / 双轨呈现。

1. ~~**向量库阶段一**~~ ✅ 完成（四因子检索 + 降级兜底，见 M1-vector-memory-progress.md）
2. ~~**存储治理落地**~~ ✅ 完成（失效标记 + 容量 + P0 保护 + 弹窗）
3. ~~**routes.jsonl 脱敏降级**~~ ✅ 完成（脱敏 + 容量轮转）
4. **遗留（不阻塞，可选）**：recall.now_fn 删除（已确认无设计意义）、~~前端双轨呈现 F1~~ ✅ 已做、M4 真实 VLM/摄像头、M5 HA 真实 REST/WebSocket、Piper GPL 边界。
