...
SCHEMA: list[dict] = [
    ...
    {"path": "tts.cloud.api_key", "label": "API Key", "type": "text", "group": "tts", "reload": "restart",
     "show_if": {"path": "tts.provider", "value": "cloud"},
    ...
    {"path": "_guide.tts.cloud", "label": "引导", "type": "guide", "group": "tts",
     "show_if": {"path": "tts.provider", "value": "cloud"},
     "guide": "付费云 TTS：音质比 edge-tts 更自然、可选专属音色。\n先到对应服务商（阿里云 / 火山引擎 / Azure）开通语音合成并获取 API Key，再在上方填 Key 与音色名。\n（当前为预留接口）"},
    {"path": "_guide.tts.piper", "label": "引导", "type": "guide", "group": "tts",
     "show_if": {"path": "tts.provider", "value": "piper"},
     "guide": "本地 Piper：完全离线合成，无需联网、隐私本地。\n自行下载 piper 引擎与中文声库（.onnx）后，在上方填模型路径与声库语音名（如 zh_CN-huayan-medium）。\n（当前为预留接口）"},
    {"path": "_guide.tts.omni", "label": "引导", "type": "guide", "group": "tts",
     "show_if": {"path": "tts.provider", "value": "omni"}, "guide": OMNI_GUIDE},
